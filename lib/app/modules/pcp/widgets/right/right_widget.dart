...
class RightWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Observer(builder: (_) {
      if (controller.status == PcpStatus.none) {
        return _buildContainer(Text("colocar conteudo geral"));
      } else if (controller.status == PcpStatus.loading) {
        return _buildContainer(_load());
      } else if (controller.status == PcpStatus.success) {
        PcpStatus statusRight = controller.status;
