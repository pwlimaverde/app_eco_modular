...
//            var formatter = new DateFormat('dd-MM-yyyy');
            ...
            String decoderReple = decoderByte.replaceAll('\n\r\n ', '').replaceAll('Série: P"', 'Série: P",\r\n').trim();
            ...
            for(String item in decoderRow){
              ...
              List<String> i2 = item.replaceAll("\r\n", " | ").trim().replaceAll('$ano,', '$ano","').replaceFirst(',', ',"').trim().split(',"');
              row["oc"] = i2[0].replaceAll("|", " ").trim();
              row["cliente"] = i2[1].replaceAll('"', '').trim();
              row["servico"] = i2[2].replaceAll('"', '').trim();
              row["quant"] = i2[3].replaceAll('"', '').replaceAll(',00', '').replaceAll('.', '').trim();
              String valor = i2[4].substring(0,i2[4].indexOf('",'));
              row["valor"] = valor;
              String entrada = i2[4].substring(i2[4].indexOf('",')+2).replaceAll('"', '').trim();
              row["entrada"] = entrada;
              String voe = i2[5];
              String vendedor = voe.substring(0, voe.indexOf(',')).replaceAll('"', '').trim();
